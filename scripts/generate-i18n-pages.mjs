...
const ORGANIZATION_LD_MARKER = 'data-pagodapdf-organization';
...
function injectOrganizationLd(document) {
  ...
  const data = {
    ...
    name: BRAND_NAME,
    url: SITE_URL,
    logo: siteAssetUrl(BRAND_LOGO),
  };
  if (BRAND_SOCIAL_LINKS.length > 0) {
    data.sameAs = BRAND_SOCIAL_LINKS;
  }
...
const BREADCRUMB_MARKER = 'data-pagodapdf-breadcrumb';
...
function processFileForLanguage(
  ...
  const result = rebrandText(dom.serialize());
