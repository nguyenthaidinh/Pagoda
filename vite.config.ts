...
export default defineConfig(() => {
  ...
  const brandName = process.env.VITE_BRAND_NAME || 'PagodaPDF';
  const brandLogo = process.env.VITE_BRAND_LOGO || 'images/pagoda-logo.svg';
  const footerText =
    process.env.VITE_FOOTER_TEXT ||
    '(c) 2026 PagodaPDF. Released under AGPL-3.0.';
