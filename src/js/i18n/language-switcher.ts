...
const footerLanguageCodes: SupportedLanguage[] = ['en', 'vi', 'de'];
...
export const initFooterLanguageSwitchers = (): void => {
  const currentLang = getLanguageFromUrl();

  document
    .querySelectorAll<HTMLElement>('[data-footer-language-switcher]')
    .forEach((container) => {
      const options =
        container.querySelectorAll<HTMLButtonElement>('[data-footer-lang]');

      options.forEach((option) => {
        const lang = option.dataset.footerLang as SupportedLanguage | undefined;
        if (!lang || !footerLanguageCodes.includes(lang)) return;

        const isActive = lang === currentLang;
        option.classList.toggle('is-active', isActive);
        option.setAttribute('aria-pressed', isActive.toString());

        if (option.dataset.footerLangReady === 'true') return;
        option.dataset.footerLangReady = 'true';

        option.addEventListener('click', () => {
          if (lang !== getLanguageFromUrl()) {
            changeLanguage(lang);
          }
        });
      });
