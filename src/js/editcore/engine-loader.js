...
function resolveWasmUrl() {
  ...
  const resolvedUrl = new URL(resolve('bentopdf-pdfium/editcore.wasm'));
  const pathname = decodeURIComponent(resolvedUrl.pathname);

  // URL.pathname keeps the leading slash in Windows file URLs (/F:/...),
  // which Emscripten otherwise resolves against cwd as F:\F:\...
