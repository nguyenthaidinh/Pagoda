...
export interface RenderConfig {
  ...
  createPlaceholder?: (pageNumber: number) => HTMLElement;
}
...
async function renderPageBatch(tasks: PageTask[]): Promise<void> {
  for (const task of tasks) {
    try {
      if (task.config?.shouldCancel?.() || !task.placeholderElement?.parentNode)
        continue;
...
function setupLazyRendering(
  ...
  if (lazyLoadState.observer) return lazyLoadState.observer;
        ...
        if (task) {
          ...
          renderPageBatch([task])
            .then(() => {
              ...
              if (task.config?.onBatchComplete) {
...
export async function renderPagesProgressively(
  ...
  for (let i = 1; i <= totalPages; i++) {
    const placeholder = config.createPlaceholder?.(i) ?? createPlaceholder(i);
...
export function observePlaceholder(
  ...
  if (!lazyLoadState.observer) {
    setupLazyRendering(task.container, task.config ?? {});
  }
  task.placeholderElement = placeholder;
  lazyLoadState.pendingTasks.set(placeholder, task);
...
function renderEagerBatch(config: RenderConfig): void {
  ...
  requestIdleCallbackPolyfill(async () => {
    ...
    const tasksToRender = batch.filter(
      (task) =>
        !!task.placeholderElement &&
        lazyLoadState.pendingTasks.has(task.placeholderElement)
    );
