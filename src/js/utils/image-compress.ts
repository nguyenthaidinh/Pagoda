...
export async function compressImageBytes(
  ...
  if (quality === 'high') {
    const isPng = [137, 80, 78, 71, 13, 10, 26, 10].every(
      (value, index) => source[index] === value
    );
    if (isPng) return { bytes: source, type: 'png' };
    if (source[0] === 255 && source[1] === 216 && source[2] === 255)
      return { bytes: source, type: 'jpeg' };
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      const type = quality === 'high' ? 'png' : 'jpeg';
      if (type === 'jpeg') {
        ctx.fillStyle = '#ffffff';
        ctx.fillRect(0, 0, width, height);
      }
      ...
      canvas.toBlob(
        async (jpegBlob) => {
          ...
          resolve({ bytes: new Uint8Array(arrayBuffer), type });
        },
        `image/${type}`,
        config.jpegQuality
      );
