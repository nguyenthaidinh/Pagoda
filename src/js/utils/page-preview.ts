...
export function initPagePreview(
  ...
  options: {
    pageAttr?: string;
    resolveDocument?: (thumbnail: HTMLElement) => PDFDocumentProxy | undefined;
  } = {}
): void {
  const previewThumbnail = (thumbnail: HTMLElement, pageNumber: number) => {
    const source = options.resolveDocument
      ? options.resolveDocument(thumbnail)
      : pdfjsDoc;
    if (source) showPreview(source, pageNumber, source.numPages);
  };
  ...
  thumbnails.forEach((thumb) => {
    ...
    icon.addEventListener('click', (e) => {
      e.stopPropagation();
      e.preventDefault();
      previewThumbnail(thumb, pageNum);
    });
  ...
  if (container.dataset.previewKeyboardInit) return;
  container.dataset.previewKeyboardInit = 'true';
  container.addEventListener('keydown', (e) => {
    if (e.key === ' ' && !state.isOpen) {
      ...
      if (hovered) {
        ...
        previewThumbnail(hovered, pageNum);
      }
    }
  });
}
