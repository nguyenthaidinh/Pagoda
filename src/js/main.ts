...
const init = async () => {
  ...
  if (__SIMPLE_MODE__) {
    const hideBrandingSections = () => {
      ...
      const githubLink = document.querySelector('[data-github-stars]');
      if (githubLink) {
      ...
      const brandName = __BRAND_NAME__ || 'PagodaPDF';
  ...
  if (dom.toolGrid) {
    ...
    filteredCategories.forEach((category) => {
      ...
      category.tools.forEach((tool) => {
        ...
        toolCard.append(iconWrapper, toolName);

        if (tool.subtitle) {
          const toolSubtitle = document.createElement('p');
          toolSubtitle.className = 'tool-subtitle';
  ...
  createIcons({ icons });
  console.log('PagodaPDF is ready.');
