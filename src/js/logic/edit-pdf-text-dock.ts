...
const editText = (key: string, fallback: string): string =>
  t(`tools:editPdfText.${key}`, { defaultValue: fallback });

const sectionLabel = (spec: SectionSpec): string => {
  const keys: Record<string, string> = {
    ...
    scope: 'dockEditScope',
    stroke: 'pathStroke',
    alignMulti: 'dockAlignObjects',
  };
  return editText(keys[spec.id] || spec.id, spec.title);
};
...
const fieldLabel = (id: string, fallback: string): string => {
  const keys: Record<string, string> = {
    ...
    fChar: 'dockCharacter',
    fWordSp: 'dockWord',
    fHScale: 'dockHorizontalScale',
    fFirstInd: 'dockFirstIndent',
    fHangInd: 'hangingIndent',
  };
  return editText(keys[id] || id, fallback);
};
...
function setupPagePill(): void {
  ...
  const pill = mkBtn('dpagepill', editText('dockGoToPage', 'Go to page'));
  pill.textContent = '– / –';
  ...
  pill.addEventListener('click', () => {
    ...
    const prevB = mkBtn(
      'dnavbtn',
      editText('tipPreviousPage', 'Previous page')
    );
    prevB.appendChild(ph('ph-caret-left'));
    prevB.addEventListener('click', () => q('prev')?.click());
    const nextB = mkBtn('dnavbtn', editText('tipNextPage', 'Next page'));
    ...
    inp.setAttribute('aria-label', editText('dockPageNumber', 'Page number'));
    const st = parse();
    ...
    const goBtn = mkBtn('dnavbtn dgo', editText('dockGoToPage', 'Go to page'));
    goBtn.textContent = editText('dockGo', 'Go');
    ...
    const props = mkBtn(
      'ecmenu-item',
      editText('documentProperties', 'Document properties')
    );
    const propLabel = document.createElement('span');
    propLabel.textContent = editText(
      'documentProperties',
      'Document properties'
    );
...
export function setupFormatDock(): void {
  ...
  const dclose = mkBtn('dclose', editText('dockClosePanel', 'Close panel'));
  ...
  const delChip = mkBtn('dchip ddel', t('common.delete'));
  ...
  if (fSize) {
    ...
    const minus = step(
      -1,
      'ph-minus',
      editText('dockDecreaseSize', 'Decrease size')
    );
    const plus = step(
      1,
      'ph-plus',
      editText('dockIncreaseSize', 'Increase size')
    );
  ...
  dirLab.textContent = editText('direction', 'Direction');
  ...
  for (const [id, label] of SLIDER_FIELDS) {
    const row = sliderRow(id, fieldLabel(id, label));
  ...
  outLab.textContent = editText('outline', 'Outline');
  ...
  const scopeDefs: [string, string, string][] = [
    ['all', 'filterAll', 'All'],
    ['text', 'filterText', 'Text'],
    ['image', 'filterImages', 'Images'],
    ['shape', 'filterShapes', 'Shapes'],
  ];
  const renderScope = () => {
    scopeWrap.replaceChildren();
    for (const [key, translationKey, fallback] of scopeDefs) {
      const label = editText(translationKey, fallback);
  ...
  const setHead = (spec: SectionSpec) => {
    ...
    dtext.textContent = sectionLabel(spec);
  };
  ...
  shead.append(
    ph('ph-text-aa'),
    document.createTextNode(` ${editText('dockSelectFont', 'Select Font')}`)
  );
  ...
  const rebuildFonts = () => {
    ...
    if ('queryLocalFonts' in window) {
      ...
      const sys = mkBtn(
        'dfrow dfsys',
        editText('dockLoadSystemFonts', 'Load system fonts')
      );
  ...
  const secChip = (spec: SectionSpec): HTMLButtonElement => {
    const b = mkBtn('dchip', sectionLabel(spec));
    ...
    if (mode !== 'idle') {
      chipsWrap.appendChild(
        toolChip(
          'done',
          'ph-check',
          editText('dockDone', 'Done — back to tools'),
          () => {
            closePanel();
            closeSheet();
            editBtn.click();
    ...
    if (mode === 'text') {
      ...
      chipsWrap.appendChild(
        toolChip(
          'edit',
          'ph-pencil-simple',
          editText('dockEditTool', 'Edit tool'),
          () => {
      ...
      chipsWrap.appendChild(
        toolChip(
          'addText',
          'ph-textbox',
          editText('addTextBox', 'Add a text box'),
          () => {
            closePanel();
            document
      ...
      chipsWrap.appendChild(
        toolChip(
          'addImage',
          'ph-image',
          editText('tipAddAnImage', 'Add an image'),
          () => {
            closePanel();
            q('addImage')?.click();
          }
      ...
      chipsWrap.appendChild(
        toolChip(
          'find',
          'ph-magnifying-glass',
          editText('tipFindAndReplace', 'Find & replace'),
          () => {
            closePanel();
            q('find')?.click();
          }
