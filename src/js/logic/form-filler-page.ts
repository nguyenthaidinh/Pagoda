...
import { getPDFDocument, isPdfFile } from '../utils/helpers.js';
...
function updateFileDisplay() {
  ...
  removeBtn.title = t('tools:pdfFormFiller.removeFile');
...
async function handleFileUpload(file: File) {
  if (!isPdfFile(file)) {
    showAlert(t('common.error'), t('tools:pdfFormFiller.invalidFile'));
    return;
  }
  ...
  try {
    ...
    console.error(error);
    showAlert(t('common.error'), t('tools:pdfFormFiller.loadFailed'));
    hideLoader();
  }
}
...
async function setupFormViewer() {
  ...
  viewerReady = false;
  showLoader(t('tools:pdfFormFiller.loadingForm'));
  ...
  try {
    ...
    viewerUrl.searchParams.set('file', currentBlobUrl);
    viewerUrl.hash = new URLSearchParams({
      locale: getLanguageFromUrl(),
    }).toString();
    viewerIframe.src = viewerUrl.toString();
    ...
    viewerIframe.onerror = () => {
      viewerReady = false;
      hideLoader();
      showAlert(t('common.error'), t('tools:pdfFormFiller.viewerLoadFailed'));
    };
...
async function processAndDownloadForm() {
  if (!viewerIframe || !viewerReady) {
    showAlert(
      t('tools:pdfFormFiller.viewerNotReadyTitle'),
      t('tools:pdfFormFiller.viewerNotReady')
    );
    return;
  }
  ...
  try {
    ...
    if (!viewerWindow) {
      ...
      showAlert(
        t('common.download'),
        t('tools:pdfFormFiller.useViewerDownload')
      );
      return;
    }
    ...
    showAlert(
      t('common.download'),
      t('tools:pdfFormFiller.viewerControlsUnavailable')
    );
  }
}
