...
export async function parseMsgFile(file: File): Promise<ParsedEmail> {
  ...
  const attachments: EmailAttachment[] = (msgData.attachments || []).map(
    (att, index) => {
      // getFileData exposes metadata; attachment bytes are read separately.
      const attachment = msgReader.getAttachment(index);
      const content = new Uint8Array(attachment.content);
      return {
        filename: attachment.fileName || att.fileName || att.name || 'unnamed',
        size: content.byteLength,
        contentType: att.attachMimeTag || 'application/octet-stream',
        content,
