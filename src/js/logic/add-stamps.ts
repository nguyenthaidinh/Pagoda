...
function updateFileList() {
  ...
  deleteBtn.title = t('tools:addStamps.removeFile');
...
async function loadPdfInViewer(file: File) {
  ...
  if (stampUserName) {
    viewerHash.set('ae_username', stampUserName);
  }
...
async function onPdfSelected(file: File) {
  if (!isPdfFile(file)) {
    alert(t('tools:addStamps.invalidFile'));
    return;
  }
  ...
  try {
    ...
    console.error('Failed to load PDF for stamping:', error);
    alert(t('tools:addStamps.loadFailed'));
    resetState();
  }
}
...
if (saveStampedBtn) {
  saveStampedBtn.addEventListener('click', () => {
    if (!viewerIframe) {
      alert(t('tools:addStamps.viewerNotReady'));
      return;
    }
    ...
    try {
      ...
      alert(t('tools:addStamps.exportFallback'));
    } catch (e) {
      console.error('Failed to trigger stamped PDF export:', e);
      alert(t('tools:addStamps.exportFailed'));
    }
  });
}
