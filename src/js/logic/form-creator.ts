...
import { embedLatinExtendedPdfFont } from '../utils/pdf-font';
import '../utils/setup-pdf-worker.js';

const formText = (key: string, options?: Record<string, unknown>): string =>
  t(`tools:createPdfForm.${key}`, options);

const safeFormText = (key: string, options?: Record<string, unknown>): string =>
  escapeHtml(formText(key, options));

function getFieldTypeLabel(type: FormField['type']): string {
  const keys: Record<FormField['type'], string> = {
    text: 'textField',
    checkbox: 'checkbox',
    radio: 'radioButton',
    dropdown: 'dropdown',
...
if (gotoPageBtn && gotoPageInput) {
  gotoPageBtn.addEventListener('click', () => {
    ...
    if (!isNaN(pageNum) && pageNum >= 1 && pageNum <= pages.length) {
      ...
      alert(formText('invalidPageNumber', { total: pages.length }));
    }
  });
  ...
  const field: FormField = {
    ...
    options:
      type === 'dropdown' || type === 'optionlist'
        ? [formText('option1'), formText('option2'), formText('option3')]
        : undefined,
...
function renderField(field: FormField): void {
  ...
  if (field.type === 'text') {
    ...
    let displayText = formText('selectPlaceholder');
    if (
      field.defaultValue &&
      field.options &&
    ...
    if (field.options && field.options.length > 0) {
      ...
      optEl.textContent = formText('item1');
      contentEl.appendChild(optEl);
    }
    ...
    contentEl.textContent = field.label || formText('button');
    ...
    contentEl.innerHTML = `<div class="flex flex-col items-center"><i data-lucide="pen-tool" class="w-6 h-6 mb-1"></i><span class="text-[10px]">${safeFormText('signHere')}</span></div>`;
    ...
    contentEl.innerHTML = `<div class="flex flex-col items-center text-center p-1"><i data-lucide="image" class="w-6 h-6 mb-1"></i><span class="text-[10px] leading-tight">${escapeHtml(field.label || formText('imagePrompt'))}</span></div>`;
    setTimeout(() => (window as LucideWindow).lucide?.createIcons(), 0);
    ...
    if (field.barcodeValue) {
      try {
        ...
        contentEl.innerHTML = `<div class="flex flex-col items-center text-center p-1 text-gray-400"><i data-lucide="qr-code" class="w-6 h-6 mb-1"></i><span class="text-[10px] leading-tight">${safeFormText('invalidBarcodeData')}</span></div>`;
        setTimeout(() => (window as LucideWindow).lucide?.createIcons(), 0);
      }
    } else {
      contentEl.innerHTML = `<div class="flex flex-col items-center text-center p-1 text-gray-400"><i data-lucide="qr-code" class="w-6 h-6 mb-1"></i><span class="text-[10px] leading-tight">${safeFormText('barcode')}</span></div>`;
      setTimeout(() => (window as LucideWindow).lucide?.createIcons(), 0);
    }
  }
...
function showProperties(field: FormField): void {
  ...
  if (field.type === 'text') {
    specificProps = `
        <div>
            <label class="block text-xs font-semibold text-gray-300 mb-1">${safeFormText('value')}</label>
        ...
        <div>
            <label class="block text-xs font-semibold text-gray-300 mb-1">${safeFormText('maxLength')}</label>
        ...
        <div>
            <label class="block text-xs font-semibold text-gray-300 mb-1">${safeFormText('divideIntoBoxes')}</label>
        ...
        <div>
            <label class="block text-xs font-semibold text-gray-300 mb-1">${safeFormText('fontSize')}</label>
        ...
        <div>
            <label class="block text-xs font-semibold text-gray-300 mb-1">${safeFormText('textColor')}</label>
            <input type="color" id="propTextColor" value="${field.textColor}">
        </div>
        <div>
            <label class="block text-xs font-semibold text-gray-300 mb-1">${safeFormText('alignment')}</label>
            ...
            <option value="left" ${field.alignment === 'left' ? 'selected' : ''}>${safeFormText('alignLeft')}</option>
            <option value="center" ${field.alignment === 'center' ? 'selected' : ''}>${safeFormText('alignCenter')}</option>
            <option value="right" ${field.alignment === 'right' ? 'selected' : ''}>${safeFormText('alignRight')}</option>
            </select>
        </div>
        <div class="flex items-center justify-between bg-gray-600 p-2 rounded mt-2">
            <label for="propMultiline" class="text-xs font-semibold text-gray-300">${safeFormText('multiline')}</label>
    ...
    specificProps = `
        <div class="flex items-center justify-between bg-gray-600 p-2 rounded">
            <label for="propChecked" class="text-xs font-semibold text-gray-300">${safeFormText('checkedState')}</label>
    ...
    specificProps = `
        <div>
            <label class="block text-xs font-semibold text-gray-300 mb-1">${safeFormText('groupName')}</label>
        ...
        <div>
            <label class="block text-xs font-semibold text-gray-300 mb-1">${safeFormText('exportValue')}</label>
    ...
    specificProps = `
        <div>
            <label class="block text-xs font-semibold text-gray-300 mb-1">${safeFormText('options')}</label>
        ...
        <div>
            <label class="block text-xs font-semibold text-gray-300 mb-1">${safeFormText('selectedOption')}</label>
            <select id="propSelectedOption" class="w-full bg-gray-600 border border-gray-500 text-white rounded px-2 py-1 text-sm focus:ring-indigo-500 focus:border-indigo-500">
                <option value="">${safeFormText('none')}</option>
        ...
        <div class="text-xs text-gray-400 italic mt-2">
            ${safeFormText('fillOptionsHint')}
        </div>
        `;
  } else if (field.type === 'button') {
    specificProps = `
        <div>
            <label class="block text-xs font-semibold text-gray-300 mb-1">${safeFormText('label')}</label>
        ...
        <div>
            <label class="block text-xs font-semibold text-gray-300 mb-1">${safeFormText('action')}</label>
            <select id="propAction" class="w-full bg-gray-600 border border-gray-500 text-white rounded px-2 py-1 text-sm focus:ring-indigo-500 focus:border-indigo-500">
                <option value="none" ${field.action === 'none' ? 'selected' : ''}>${safeFormText('none')}</option>
                <option value="reset" ${field.action === 'reset' ? 'selected' : ''}>${safeFormText('resetForm')}</option>
                <option value="print" ${field.action === 'print' ? 'selected' : ''}>${safeFormText('printForm')}</option>
                <option value="url" ${field.action === 'url' ? 'selected' : ''}>${safeFormText('openUrl')}</option>
                <option value="js" ${field.action === 'js' ? 'selected' : ''}>${safeFormText('runJavascript')}</option>
                <option value="showHide" ${field.action === 'showHide' ? 'selected' : ''}>${safeFormText('showHideField')}</option>
            </select>
        </div>
        ...
        <div id="propJsContainer" class="${field.action === 'js' ? '' : 'hidden'}">
            <label class="block text-xs font-semibold text-gray-300 mb-1">${safeFormText('javascriptCode')}</label>
        ...
        <div id="propShowHideContainer" class="${field.action === 'showHide' ? '' : 'hidden'}">
            <div class="mb-2">
                <label class="block text-xs font-semibold text-gray-300 mb-1">${safeFormText('targetField')}</label>
                <select id="propTargetField" class="w-full bg-gray-600 border border-gray-500 text-white rounded px-2 py-1 text-sm focus:ring-indigo-500 focus:border-indigo-500">
                    <option value="">${safeFormText('selectField')}</option>
                    ${fields
                      .filter((f) => f.id !== field.id)
                      .map(
                        (f) =>
                          `<option value="${escapeHtml(f.name)}" ${field.targetFieldName === f.name ? 'selected' : ''}>${escapeHtml(f.name)} (${escapeHtml(getFieldTypeLabel(f.type))})</option>`
                      )
                      .join('')}
                </select>
            </div>
            <div>
                <label class="block text-xs font-semibold text-gray-300 mb-1">${safeFormText('visibility')}</label>
                <select id="propVisibilityAction" class="w-full bg-gray-600 border border-gray-500 text-white rounded px-2 py-1 text-sm focus:ring-indigo-500 focus:border-indigo-500">
                    <option value="show" ${field.visibilityAction === 'show' ? 'selected' : ''}>${safeFormText('show')}</option>
                    <option value="hide" ${field.visibilityAction === 'hide' ? 'selected' : ''}>${safeFormText('hide')}</option>
                    <option value="toggle" ${field.visibilityAction === 'toggle' ? 'selected' : ''}>${safeFormText('toggle')}</option>
                </select>
            </div>
        </div>
        `;
    ...
    specificProps = `
        <div>
            <label class="block text-xs font-semibold text-gray-300 mb-1">${safeFormText('dateFormat')}</label>
            <select id="propDateFormat" class="w-full bg-gray-600 border border-gray-500 text-white rounded px-2 py-1 text-sm focus:ring-indigo-500 focus:border-indigo-500">
                ...
                <option value="custom" ${isCustom ? 'selected' : ''}>${safeFormText('custom')}</option>
            </select>
        </div>
        <div id="customFormatContainer" class="${isCustom ? '' : 'hidden'} mt-2">
            <label class="block text-xs font-semibold text-gray-300 mb-1">${safeFormText('customFormat')}</label>
        ...
        <div class="mt-3 p-2 bg-gray-700 rounded">
            <span class="text-xs text-gray-400">${safeFormText('formatExample')}</span>
        ...
        <div class="bg-blue-900/30 border border-blue-700/50 rounded p-2 mt-2">
            <p class="text-xs text-blue-200">
                ...
                <span><strong>${safeFormText('browserNoteTitle')}</strong> ${safeFormText('browserNote')}</span>
            </p>
        </div>
        `;
  } else if (field.type === 'image') {
    specificProps = `
        <div>
            <label class="block text-xs font-semibold text-gray-300 mb-1">${safeFormText('labelPrompt')}</label>
            <input type="text" id="propLabel" value="${escapeHtml(field.label)}" class="w-full bg-gray-600 border border-gray-500 text-white rounded px-2 py-1 text-sm focus:ring-indigo-500 focus:border-indigo-500">
        </div>
        <div class="text-xs text-gray-400 italic mt-2">
            ${safeFormText('imageHint')}
        </div>
        `;
  } else if (field.type === 'barcode') {
    specificProps = `
        <div>
            <label class="block text-xs font-semibold text-gray-300 mb-1">${safeFormText('barcodeFormat')}</label>
        ...
        <div>
            <label class="block text-xs font-semibold text-gray-300 mb-1">${safeFormText('barcodeValue')}</label>
  ...
  const propertiesHtml = `
    <div class="space-y-3">
      <div>
        <label class="block text-xs font-semibold text-gray-300 mb-1">${safeFormText('fieldName')} ${field.type === 'radio' ? safeFormText('groupNameSuffix') : ''}</label>
      ...
      ${
        ...
        (existingRadioGroups.size > 0 ||
          ...
          ? `
      <div>
        <label class="block text-xs font-semibold text-gray-300 mb-1">${safeFormText('existingRadioGroups')}</label>
        <select id="existingGroups" class="w-full bg-gray-600 border border-gray-500 text-white rounded px-2 py-1 text-sm focus:ring-indigo-500 focus:border-indigo-500">
          <option value="">${safeFormText('selectExistingGroup')}</option>
        ...
        <p class="text-xs text-gray-400 mt-1">${safeFormText('existingGroupHint')}</p>
      </div>
      `
          : ''
      }
      ${specificProps}
      <div>
        <label class="block text-xs font-semibold text-gray-300 mb-1">${safeFormText('tooltip')}</label>
        <input type="text" id="propTooltip" value="${escapeHtml(field.tooltip)}" placeholder="${safeFormText('tooltipPlaceholder')}" class="w-full bg-gray-600 border border-gray-500 text-white rounded px-2 py-1 text-sm focus:ring-indigo-500 focus:border-indigo-500">
      </div>
      <div class="flex items-center">
        <input type="checkbox" id="propRequired" ${field.required ? 'checked' : ''} class="mr-2">
        <label for="propRequired" class="text-xs font-semibold text-gray-300">${safeFormText('required')}</label>
      </div>
      <div class="flex items-center">
        <input type="checkbox" id="propReadOnly" ${field.readOnly ? 'checked' : ''} class="mr-2">
        <label for="propReadOnly" class="text-xs font-semibold text-gray-300">${safeFormText('readOnly')}</label>
      </div>
      <div>
        <label class="block text-xs font-semibold text-gray-300 mb-1">${safeFormText('borderColor')}</label>
      ...
      <div class="flex items-center">
        ...
        <label for="propHideBorder" class="text-xs font-semibold text-gray-300">${safeFormText('hideBorder')}</label>
      </div>
      <div class="flex items-center">
        <input type="checkbox" id="propTransparentBackground" ${field.transparentBackground ? 'checked' : ''} class="mr-2">
        <label for="propTransparentBackground" class="text-xs font-semibold text-gray-300">${safeFormText('transparentBackground')}</label>
      </div>
      <button id="deleteBtn" class="w-full bg-red-600 text-white py-2 rounded hover:bg-red-700 transition text-sm font-semibold">
        ${safeFormText('deleteField')}
      </button>
    </div>
  `;
  ...
  const validateName = (newName: string): boolean => {
    if (!newName) {
      nameError.textContent = formText('fieldNameRequired');
    ...
    if (isDuplicateInFields || isDuplicateInPdf) {
      nameError.textContent = formText(
        isDuplicateInPdf ? 'fieldNameExistsPdf' : 'fieldNameExistsForm',
        { name: newName }
      );
      nameError.classList.remove('hidden');
      propName.classList.add('border-red-500');
  ...
  if (field.type === 'text') {
    ...
    const formatDateExample = (format: string): string => {
      ...
      const mmmm = new Intl.DateTimeFormat(locale, {
        month: 'long',
      }).format(now);
    ...
    const barcodeFormatHints: Record<string, string> = {
      qrcode: formText('barcodeHintAny'),
      code128: formText('barcodeHintAscii'),
      code39: formText('barcodeHintCode39'),
      ean13: formText('barcodeHintEan13'),
      upca: formText('barcodeHintUpca'),
      datamatrix: formText('barcodeHintAny'),
      pdf417: formText('barcodeHintAny'),
    };
...
function hideProperties(): void {
  propertiesPanel.innerHTML = `<p class="text-gray-500 text-sm">${safeFormText('selectFieldHint')}</p>`;
}
...
downloadBtn.addEventListener('click', async () => {
  ...
  if (conflictsWithPdf.length > 0) {
    const conflictList = [...new Set(conflictsWithPdf)]
      ...
      .join(', ');
    showModal(
      formText('fieldConflictTitle'),
      formText('fieldConflictMessage', { names: conflictList }),
      'error'
    );
    return;
  }
  ...
  if (duplicates.length > 0) {
    ...
    showModal(
      formText('duplicateFieldsTitle'),
      formText('duplicateFieldsMessage', { names: duplicateList }),
      'error'
    );
    return;
  }
  ...
  if (fields.length === 0) {
    alert(formText('addFieldFirst'));
    return;
  }

  if (pages.length === 0) {
    alert(formText('noPages'));
    return;
  }
  ...
  try {
    ...
    const formFont = await embedLatinExtendedPdfFont(pdfDoc, false);

    // Set document metadata for accessibility
    pdfDoc.setTitle(formText('documentTitle'));
    pdfDoc.setAuthor('PagodaPDF');
    ...
    pdfDoc.setLanguage(documentLanguages[currentLanguage] || currentLanguage);
    ...
    for (const field of fields) {
      ...
      if (field.type === 'text') {
        ...
        const borderRgb = hexToRgb(field.borderColor || '#000000');
        button.addToPage(field.label || formText('button'), pdfPage, {
          font: formFont,
          x: x,
          y: y,
          width: width,
        ...
        const borderRgb = hexToRgb(field.borderColor || '#000000');
        imageBtn.addToPage(field.label || formText('imagePrompt'), pdfPage, {
          font: formFont,
          x: x,
          y: y,
          width: width,
    ...
    if (
      ...
      const fieldName = match ? match[1] : formText('unknownField');

      if (existingRadioGroups.has(fieldName)) {
        console.log(`Adding to existing radio group: ${fieldName}`);
      } else {
        showModal(
          formText('duplicateFieldTitle'),
          formText('duplicateFieldMessage', { name: fieldName }),
          'error'
        );
      }
    } else {
      showModal(
        t('common.error'),
        formText('generateFailed', { error: errorMessage }),
        'error'
      );
    }
  }
  ...
  switch (size) {
    ...
    case 'custom': {
      const width = Math.min(
        14400,
        Math.max(72, parseInt(customWidth.value) || 612)
      );
      const height = Math.min(
        14400,
        Math.max(72, parseInt(customHeight.value) || 792)
      );
      return { width, height };
    }
    default:
...
function resetToInitial(): void {
  ...
  currentPageIndex = 0;
  uploadedPdfDoc = null;
  uploadedPdfjsDoc?.destroy();
  uploadedPdfjsDoc = null;
  uploadedFileName = null;
  selectedField = null;
  fieldCounter = 0;
  existingFieldNames.clear();
  existingRadioGroups.clear();
...
function updatePageNavigation(): void {
  pageIndicator.textContent = formText('pageInfo', {
...
dropZone.addEventListener('drop', (e) => {
  ...
  if (files && files.length > 0) {
    if (isPdfFile(files[0])) {
      ...
      showModal(t('common.error'), formText('invalidFile'), 'error');
    }
  }
});
...
async function handlePdfUpload(file: File) {
  ...
  try {
    ...
    showModal(t('common.error'), formText('loadFailed'), 'error');
  }
}
...
resetBtn.addEventListener('click', () => {
  if (fields.length > 0 || pages.length > 0) {
    if (confirm(formText('resetConfirm'))) {
      resetToInitial();
    }
  } else {
    resetToInitial();
...
function showModal(
  ...
  onClose?: () => void,
  buttonText: string = formText('close')
) {
