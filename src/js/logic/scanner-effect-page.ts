...
function clearPreviewDocument(): void {
  pdfjsDoc?.destroy();
  pdfjsDoc = null;
}
...
const updateUI = () => {
  ...
  if (files.length > 0) {
    ...
    files.forEach((file) => {
      ...
      metaSpan.textContent = pdfjsDoc
        ? t('tools:scannerEffect.fileMeta', {
      ...
      removeBtn.title = t('tools:scannerEffect.removeFile');
      removeBtn.onclick = () => {
        files = [];
...
async function processAllPages(): Promise<void> {
  if (files.length === 0) {
    showAlert(t('common.error'), t('tools:scannerEffect.uploadFirst'));
    return;
  }

  showLoader(t('tools:scannerEffect.processing'));

  let doc: pdfjsLib.PDFDocumentProxy | null = null;
  try {
    const settings = getSettings();
    const pdfBytes = (await readFileAsArrayBuffer(files[0])) as ArrayBuffer;
    doc = await getPDFDocument({ data: pdfBytes }).promise;
    const newPdfDoc = await PDFDocument.create();
    const dpiScale = settings.resolution / 72;

    for (let i = 1; i <= doc.numPages; i++) {
      showLoader(
        t('tools:scannerEffect.processingPage', {
          current: i,
          total: doc.numPages,
        })
      );
      ...
      const jpegBlob = await canvasToBlob(outputCanvas, 'image/jpeg', 0.85);
    ...
    console.error(e);
    showAlert(t('common.error'), t('tools:scannerEffect.failed'));
  } finally {
    doc?.destroy();
    hideLoader();
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const handleFileSelect = async (newFiles: FileList | null) => {
    ...
    const validFiles = Array.from(newFiles).filter((file) => isPdfFile(file));

    if (validFiles.length === 0) {
      showAlert(t('common.error'), t('tools:scannerEffect.invalidFile'));
      return;
    }
    ...
    try {
      ...
      if (!result) return;
      showLoader(t('tools:scannerEffect.loadingPreview'));
      clearPreviewDocument();
      files = [result.file];
      pdfjsDoc = result.pdf;
      updateUI();
      ...
      console.error(e);
      showAlert(t('common.error'), t('tools:scannerEffect.loadFailed'));
    } finally {
      hideLoader();
    }
  };
