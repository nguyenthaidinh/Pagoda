...
import { isPdfFile, securityText } from '../utils/security-pdf-page.js';
...
async function handleCertFile(file: File): Promise<void> {
  const hasValidExtension = /\.(pem|crt|cer|der)$/i.test(file.name);

  if (!hasValidExtension) {
    showAlert(
      t('tools:validateSignaturePdf.invalidCertificateTitle'),
      t('tools:validateSignaturePdf.invalidCertificateMessage')
    );
    return;
  }
  ...
  try {
    ...
    showAlert(
      t('tools:validateSignaturePdf.invalidCertificateTitle'),
      t('tools:validateSignaturePdf.failedParseCertificate')
    );
    resetCertState();
  }
}
...
function updateCertDisplay(): void {
  ...
  metaSpan.append(
    ...
    document.createTextNode(
      t('tools:validateSignaturePdf.trustedCertificateLoaded')
    )
  );
...
async function validateSignatures(): Promise<void> {
  ...
  showLoader(t('tools:validateSignaturePdf.analyzing'));
  ...
  try {
    ...
    showAlert(
      ...
      t('tools:validateSignaturePdf.validationFailed')
    );
  } finally {
    hideLoader();
  }
...
function displayResults(): void {
  ...
  if (state.results.length === 0) {
    resultsContainer.innerHTML = `
            <div class="bg-gray-700 rounded-lg p-6 text-center border border-gray-600">
                ...
                <h3 class="text-lg font-semibold text-white mb-2">${escapeHtml(t('tools:validateSignaturePdf.noSignaturesTitle'))}</h3>
                <p class="text-gray-400">${escapeHtml(t('tools:validateSignaturePdf.noSignaturesMessage'))}</p>
            </div>
        `;
    createIcons({ icons });
    return;
  ...
  let summaryHtml = `
        <p class="text-gray-300">
            ...
            ${escapeHtml(t('tools:validateSignaturePdf.signaturesFound', { count: state.results.length }))}
            <span class="text-gray-500">&bull;</span>
            <span class="${validCount === state.results.length ? 'text-green-400' : 'text-yellow-400'}">${escapeHtml(t('tools:validateSignaturePdf.validCount', { count: validCount }))}</span>
        </p>
    `;
  ...
  if (state.trustedCert) {
    summaryHtml += `
            <p class="text-xs text-gray-400 mt-1">
                ...
                ${escapeHtml(t('tools:validateSignaturePdf.trustVerification', { verified: trustVerified, total: state.results.length }))}
            </p>
        `;
  }
...
function createSignatureCard(
  ...
  let statusText = t('tools:validateSignaturePdf.validSignature');

  if (result.usesInsecureDigest) {
    statusColor = 'text-red-400';
    statusIcon = 'x-circle';
    statusText = t('tools:validateSignaturePdf.insecureDigest');
  } else if (!result.isValid) {
    if (result.cryptoVerificationStatus === 'unsupported') {
      statusColor = 'text-yellow-400';
      statusIcon = 'alert-triangle';
      statusText = t('tools:validateSignaturePdf.unsupportedAlgorithm');
    } else if (
      ...
      statusColor = 'text-red-400';
      statusIcon = 'x-circle';
      statusText = t('tools:validateSignaturePdf.modifiedAfterSigning');
    } else {
      statusColor = 'text-red-400';
      statusIcon = 'x-circle';
      statusText =
        result.cryptoVerified === false
          ? t('tools:validateSignaturePdf.cryptoVerificationFailed')
          : t('tools:validateSignaturePdf.invalidSignature');
    }
  } else if (result.isExpired) {
    statusColor = 'text-yellow-400';
    statusIcon = 'alert-triangle';
    statusText = t('tools:validateSignaturePdf.certificateExpired');
  } else if (!result.isTrusted) {
    statusColor = 'text-yellow-400';
    statusIcon = 'alert-triangle';
    statusText = result.isSelfSigned
      ? t('tools:validateSignaturePdf.selfSignedUnverified')
      : t('tools:validateSignaturePdf.identityUnverified');
  }

  const formatDate = (date: Date) => {
    if (!date || date.getTime() === 0)
      return t('tools:validateSignaturePdf.unknown');
  ...
  if (state.trustedCert) {
    if (result.isTrusted) {
      trustBadge = `<span class="text-xs bg-green-900 text-green-300 px-2 py-1 rounded ml-2"><i data-lucide="shield-check" class="inline w-3 h-3 mr-1"></i>${escapeHtml(t('tools:validateSignaturePdf.trusted'))}</span>`;
    } else {
      trustBadge = `<span class="text-xs bg-gray-600 text-gray-300 px-2 py-1 rounded ml-2"><i data-lucide="shield-x" class="inline w-3 h-3 mr-1"></i>${escapeHtml(t('tools:validateSignaturePdf.notInTrustChain'))}</span>`;
    }
  }
  ...
  card.innerHTML = `
        <div class="flex items-start justify-between mb-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="font-semibold text-white">${escapeHtml(t('tools:validateSignaturePdf.signatureNumber', { number: index + 1 }))}</h3>
                    <p class="text-sm ${statusColor}">${escapeHtml(statusText)}</p>
                </div>
            </div>
            <div class="flex items-center">
                ${
                  result.coverageStatus === 'full'
                    ? `<span class="text-xs bg-green-900 text-green-300 px-2 py-1 rounded">${escapeHtml(t('tools:validateSignaturePdf.fullCoverage'))}</span>`
                    : result.coverageStatus === 'partial'
                      ? `<span class="text-xs bg-yellow-900 text-yellow-300 px-2 py-1 rounded">${escapeHtml(t('tools:validateSignaturePdf.partialCoverage'))}</span>`
                      : ''
                }${trustBadge}
            </div>
        </div>
        ...
        <div class="space-y-3 text-sm">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <p class="text-gray-400">${escapeHtml(t('tools:validateSignaturePdf.signedBy'))}</p>
                ...
                <div>
                    <p class="text-gray-400">${escapeHtml(t('tools:validateSignaturePdf.issuer'))}</p>
            ...
            ${
              result.signatureDate
                ? `
                <div>
                    <p class="text-gray-400">${escapeHtml(t('tools:validateSignaturePdf.signedOn'))}</p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <p class="text-gray-400">${escapeHtml(t('tools:validateSignaturePdf.validFrom'))}</p>
                    <p class="text-white">${formatDate(result.validFrom)}</p>
                </div>
                <div>
                    <p class="text-gray-400">${escapeHtml(t('tools:validateSignaturePdf.validUntil'))}</p>
            ...
            ${
              result.reason
                ? `
                <div>
                    <p class="text-gray-400">${escapeHtml(t('tools:validateSignaturePdf.reason'))}</p>
            ...
            ${
              result.location
                ? `
                <div>
                    <p class="text-gray-400">${escapeHtml(t('tools:validateSignaturePdf.location'))}</p>
            ...
            <details class="mt-2">
                <summary class="cursor-pointer text-indigo-400 hover:text-indigo-300 text-sm">
                    ${escapeHtml(t('tools:validateSignaturePdf.technicalDetails'))}
                </summary>
                <div class="mt-2 p-3 bg-gray-800 rounded text-xs space-y-1">
                    <p><span class="text-gray-400">${escapeHtml(t('tools:validateSignaturePdf.serialNumber'))}:</span> <span class="text-gray-300 font-mono">${escapeHtml(result.serialNumber)}</span></p>
                    <p><span class="text-gray-400">${escapeHtml(t('tools:validateSignaturePdf.digestAlgorithm'))}:</span> <span class="text-gray-300">${escapeHtml(result.algorithms.digest)}</span></p>
                    <p><span class="text-gray-400">${escapeHtml(t('tools:validateSignaturePdf.signatureAlgorithm'))}:</span> <span class="text-gray-300">${escapeHtml(result.algorithms.signature)}</span></p>
                    ${result.errorMessage ? `<p class="text-red-400">${escapeHtml(t('tools:validateSignaturePdf.errorLabel'))}: ${escapeHtml(result.errorMessage)}</p>` : ''}
