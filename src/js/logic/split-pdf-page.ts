    ...
    if (state.files.length > 0) {
      ...
      if (fileDisplayArea) {
        ...
        try {
          ...
          metaSpan.textContent = `${formatBytes(file.size)} • ${pageCount} ${t('common.pages')}`;
        } catch (error) {
          console.error('Error loading PDF:', error);
          showAlert(t('common.error'), t('tools:splitPdf.loadFailed'));
          state.files = [];
          updateUI();
          return;
        }
    ...
    try {
      ...
      await renderPagesProgressively(pdf, container, createWrapper, {
        ...
        onProgress: (current, total) => {
          showLoader(t('tools:splitPdf.renderingPreviews', { current, total }));
        },
        onBatchComplete: () => {
          createIcons({ icons });
        },
      ...
      showAlert(t('common.error'), t('tools:splitPdf.previewFailed'));
  ...
  const split = async () => {
    ...
    try {
      ...
      switch (splitMode) {
        ...
        case 'bookmarks': {
          ...
          if (splitPages.length === 0) {
            throw new Error(t('tools:splitPdf.noBookmarks'));
          }
        ...
        case 'n-times': {
          ...
          if (nValue < 1) throw new Error(t('tools:splitPdf.minimumPages'));
      ...
      if (
        ...
        throw new Error(t('tools:splitPdf.noPagesSelected'));
      }
      ...
      if (outputGroups && outputGroups.length > 0) {
        if (outputGroups.length === 1) {
          ...
          showLoader(t('tools:splitPdf.creatingZip'));
          const zip = new JSZip();
      ...
      showAlert(
        t('common.error'),
        e instanceof Error ? e.message : t('tools:splitPdf.splitFailed')
      );
    } finally {
      if (qpdf) {
        try {
    ...
    splitModeSelect.addEventListener('change', (e) => {
      ...
      if (mode === 'range') {
        ...
        if (outputSeparateLabel)
          outputSeparateLabel.textContent = t('tools:splitPdf.outputPerRange');
      } else if (mode === 'visual') {
        visualPanel?.classList.remove('hidden');
        outputModeWrapper?.classList.remove('hidden');
        if (outputSeparateLabel)
          outputSeparateLabel.textContent = t('tools:splitPdf.outputPerPage');
        ...
        const updateWarning = () => {
          ...
          if (remainder !== 0 && nTimesWarning) {
            ...
            if (warningText) {
              warningText.textContent = t('tools:splitPdf.unevenGroups', {
                total: totalPages,
                size: nValue,
                remainder,
              });
