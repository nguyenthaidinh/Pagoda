...
async function initializePage() {
  ...
  document.getElementById('run-btn')?.addEventListener('click', async () => {
    ...
    if (allNodes.length === 0) {
      showAlert(t('common.error'), t('tools:pdfWorkflow.addNodeError'));
      return;
    }
    const hasInput = allNodes.some((n) => n.category === 'Input');
    const hasOutput = allNodes.some((n) => n.category === 'Output');
    if (!hasInput || !hasOutput) {
      showAlert(t('common.error'), t('tools:pdfWorkflow.needInputOutput'));
      return;
    }
    ...
    try {
      ...
      if (statusText)
        statusText.textContent = t('tools:pdfWorkflow.workflowCompleted');
    } catch (err) {
      if (statusText)
        statusText.textContent = t('tools:pdfWorkflow.errorDuringExecution');
  ...
  document.getElementById('clear-btn')?.addEventListener('click', async () => {
    ...
    updateNodeCount();
    const statusText = document.getElementById('status-text');
    if (statusText) statusText.textContent = t('tools:pdfWorkflow.ready');
...
function updateNodeCount() {
  ...
  if (el) el.textContent = t('tools:pdfWorkflow.nodeCount', { count });
}
...
function showSaveTemplateModal(
  ...
  const doSave = () => {
    ...
    if (!name) {
      errorEl.textContent = t('tools:pdfWorkflow.enterName');
      errorEl.classList.remove('hidden');
      return;
    }
    if (templateNameExists(name)) {
      errorEl.textContent = t('tools:pdfWorkflow.templateExists');
      errorEl.classList.remove('hidden');
      return;
    }
    ...
    showAlert(
      t('common.success'),
      t('tools:pdfWorkflow.templateSaved', { name }),
      'success'
    );
  };
...
function showLoadTemplateModal(
  ...
  if (names.length === 0) {
    ...
    for (const name of names) {
      ...
      loadBtn.textContent = t('tools:pdfWorkflow.load');
      loadBtn.addEventListener('click', async () => {
        ...
        if (loaded) {
          ...
          showAlert(
            t('common.success'),
            t('tools:pdfWorkflow.templateLoaded', { name }),
            'success'
          );
        } else {
          showAlert(
            t('common.error'),
            t('tools:pdfWorkflow.failedLoadTemplate')
          );
        }
      });
...
function showNodeSettings(node: BaseWorkflowNode) {
  ...
  if (node instanceof PDFInputNode) {
    ...
    label.textContent = t('tools:pdfWorkflow.pdfFiles');
    ...
    uploadBtn.textContent = node.hasFile()
      ? t('tools:pdfWorkflow.addMoreFiles')
      : t('tools:pdfWorkflow.uploadPdfs');
            ...
            if (password) {
              try {
                ...
                showAlert(
                  ...
                  translateWorkflowText(
                    'wrongPasswordFile',
                    'Wrong password or failed to decrypt "{{file}}".',
                    { file: file.name }
                  )
                );
              }
  ...
  if (node instanceof ImageInputNode) {
    ...
    const label = document.createElement('label');
    label.className = 'block text-xs text-gray-400 mb-1';
    label.textContent = t('tools:pdfWorkflow.images');
    fileSection.appendChild(label);

    const formatHint = document.createElement('p');
    formatHint.className = 'text-[10px] text-gray-500 mb-2';
    formatHint.textContent = translateWorkflowText(
      'supportedFormats',
      'Supported: {{formats}}',
      {
        formats:
          'JPG, PNG, BMP, GIF, TIFF, WebP, HEIC, PSD, SVG, PNM, PGM, PBM, PPM, PAM, JXR, JPX, JP2',
      }
    );
    ...
    uploadBtn.className =
      'w-full bg-gray-700 hover:bg-gray-600 text-white text-xs px-3 py-2 rounded-lg transition-colors';
    uploadBtn.textContent = node.hasFile()
      ? t('tools:pdfWorkflow.addMoreImages')
      : t('tools:pdfWorkflow.uploadImages');
  ...
  if (node instanceof DigitalSignNode) {
    ...
    certLabel.textContent = t('tools:pdfWorkflow.certificate');
    ...
    if (node.hasCertFile()) {
      ...
      removeBtn.textContent = t('tools:pdfWorkflow.remove');
      ...
      if (node.needsPassword()) {
        ...
        pwLabel.textContent = t('tools:pdfWorkflow.certificatePassword');
        ...
        pwInput.placeholder = t('tools:pdfWorkflow.enterPassword');
        pwInput.className =
        ...
        unlockBtn.textContent = t('tools:pdfWorkflow.unlock');
        ...
        const doUnlock = async () => {
          ...
          if (!pw) return;
          unlockBtn.textContent = t('tools:pdfWorkflow.unlocking');
          ...
          if (success) {
            ...
            statusMsg.textContent = t('tools:pdfWorkflow.incorrectPassword');
        ...
        okMsg.textContent = t('tools:pdfWorkflow.certificateUnlocked');
    ...
    uploadBtn.textContent = node.hasCertFile()
      ? t('tools:pdfWorkflow.replaceCertificate')
      : t('tools:pdfWorkflow.uploadCertificate');
  ...
  if (fileInputConfig) {
    ...
    const fileType = translateWorkflowText(
      `fileTypes.${fileInputConfig.key}`,
    ...
    if (fileInputConfig.hint) {
      ...
      hint.textContent = translateWorkflowText(
        'supportedFormats',
        'Supported: {{formats}}',
        { formats: fileInputConfig.hint.replace(/^Supported:\s*/, '') }
      );
    ...
    uploadBtn.textContent = fileNode.hasFile()
      ? translateWorkflowText('addMoreFilesOfType', 'Add More {{type}}', {
          type: fileType,
        })
      : translateWorkflowText('uploadFiles', 'Upload {{type}}', {
          type: fileType,
        });
  ...
  if (controlEntries.length === 0) {
    ...
    empty.textContent = t('tools:pdfWorkflow.noSettings');
  ...
  const dropdownOptions: Record<string, { label: string; value: string }[]> = {
    ...
    language: getAvailableTesseractLanguageEntries().map(([code, name]) => ({
      label: localizeOcrLanguage(code, name),
      value: code,
    })),
    gridMode: [
  ...
  ): string => translateWorkflowOptionLabel(key, option.value, option.label);
  ...
  for (const [key, control] of controlEntries) {
    ...
    if (dropdownOptions[key] && multiSelectDropdowns.has(key)) {
      ...
      searchInput.placeholder = t('tools:pdfWorkflow.searchLanguages');
      ...
      function updateTags() {
        ...
        for (const val of selectedValues) {
          ...
          tag.textContent = optionLabel(key, opt);
          const removeBtn = document.createElement('button');
      ...
      function renderList(filter: string) {
        ...
        for (const opt of dropdownOptions[key]) {
          const translatedLabel = optionLabel(key, opt);
          if (
            lowerFilter &&
            !translatedLabel.toLowerCase().includes(lowerFilter) &&
          ...
          label.appendChild(document.createTextNode(translatedLabel));
      ...
      for (const opt of dropdownOptions[key]) {
        ...
        option.textContent = optionLabel(key, opt);
  ...
  if (hasAdvanced && advancedWrappers.length > 0) {
    ...
    summaryText.textContent = t('tools:pdfWorkflow.advancedSettings');
