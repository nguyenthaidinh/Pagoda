import { downloadFile, formatBytes, isPdfFile } from '../utils/helpers';
...
interface TOCErrorResponse {
  status: 'error';
  message: string;
  code?: 'noBookmarks' | 'cpdfMissing' | 'cpdfLoadFailed' | 'generationFailed';
}
...
async function handleFileSelect(file: File) {
  if (!isPdfFile(file)) {
    showStatus(t('tools:tableOfContents.invalidPdf'), 'error');
    return;
  }
  ...
  try {
    ...
    showStatus(t('tools:tableOfContents.loadFailed'), 'error');
  }
}
...
worker.onmessage = (e: MessageEvent<TOCWorkerResponse>) => {
  ...
  if (e.data.status === 'success') {
    ...
    const errorKey = e.data.code
      ? `tools:tableOfContents.${e.data.code}`
      : 'tools:tableOfContents.generationFailed';
    showStatus(t(errorKey), 'error');
  }
};
