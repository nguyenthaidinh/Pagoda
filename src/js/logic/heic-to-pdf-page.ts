...
async function convert() {
  if (files.length === 0) {
    showAlert(
      ...
      conversionText('noFiles', { format: 'HEIC/HEIF' })
    );
    return;
  }
  showLoader(conversionText('converting', { format: 'HEIC/HEIF' }));
  try {
    ...
    showAlert(
      ...
      conversionText('failed', { format: 'HEIC/HEIF' })
    );
  } finally {
    hideLoader();
  }
  ...
  const handleFileSelect = (newFiles: FileList | null) => {
    ...
    if (validFiles.length < newFiles.length) {
      showAlert(
        ...
        conversionText('invalidFiles', { formats: 'HEIC/HEIF' })
      );
    }
