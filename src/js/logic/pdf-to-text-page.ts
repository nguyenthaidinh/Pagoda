...
async function extractText() {
  ...
  try {
    ...
    showLoader(pdfExportText('extractingText'));

    if (files.length === 1) {
      const file = files[0];
      showLoader(pdfExportText('extractingTextFile', { file: file.name }));
      ...
      showAlert(
        ...
        pdfExportText('textExtracted'),
        'success',
        resetState
      );
    } else {
      showLoader(pdfExportText('extractingTextMultiple'));
      ...
      for (let i = 0; i < files.length; i++) {
        const file = files[i];
        showLoader(
          pdfExportText('extractingTextProgress', {
            current: i + 1,
            total: files.length,
            file: file.name,
          })
      ...
      showAlert(
        pdfExportText('extractionCompleteTitle'),
        pdfExportText('textFilesExtracted', { count: files.length }),
        'success',
        () => {
          resetState();
        }
