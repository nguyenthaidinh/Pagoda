...
function clearSignaturePreviewUrl(): void {
...
function initializePage(): void {
        ...
        if (sigImageThumb && sigImagePreview) {
          ...
          try {
            ...
            if (parsed.protocol === 'blob:') {
              signaturePreviewUrl = parsed.href;
  ...
  if (removeSigImage && sigImagePreview) {
    removeSigImage.addEventListener('click', () => {
      ...
      state.sigImageType = null;
      clearSignaturePreviewUrl();
      if (sigImageThumb) sigImageThumb.removeAttribute('src');
...
async function handleCertFile(file: File): Promise<void> {
  const hasValidExtension = /\.(pfx|p12|pem)$/i.test(file.name);
...
async function handlePasswordInput(): Promise<void> {
  ...
  try {
    ...
    if (certStatus) {
      ...
      certStatus.textContent = errorMessage.toLowerCase().includes('password')
...
async function processSignature(): Promise<void> {
  ...
  if (enableVisibleSig?.checked) {
    ...
    if (
      ![sigX, sigY, sigWidth, sigHeight].every(Number.isFinite) ||
      sigX < 0 ||
      sigY < 0 ||
      sigWidth <= 0 ||
      sigHeight <= 0
      ...
      showAlert(
        t('tools:digitalSignPdf.invalidPositionTitle'),
        t('tools:digitalSignPdf.invalidPositionMessage')
      );
      return;
    }
    ...
    if (sigPageSelect) {
      if (sigPageSelect.value === 'last') {
        ...
        if (
          ...
          showAlert(
            t('tools:digitalSignPdf.invalidPageTitle'),
            t('tools:digitalSignPdf.invalidPageMessage', { count: numPages })
          );
          return;
        }
        sigPage = requestedPage - 1;
      } else {
  ...
  try {
    ...
    downloadFile(
      blob,
      pdfOutputName(state.pdfFile?.name ?? 'document.pdf', 'signed')
    );
