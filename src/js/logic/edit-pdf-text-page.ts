...
const SCOPES: [string, string, string, string][] = [
  ['text', 'editText', 'Edit Text', 'ph-text-t'],
  ['image', 'editImages', 'Edit Images', 'ph-image'],
  ['shape', 'editShapes', 'Edit Shapes', 'ph-shapes'],
];
...
function setupEditMenu() {
  ...
  const apply = () => {
    ...
    label.textContent = picked.has('all')
      ? editText('editAll', 'Edit All')
      : SCOPES.filter(([key]) => picked.has(key))
          .map(([, translationKey, fallback]) =>
            editText(translationKey, fallback)
          )
          .join(' + ');
  };
  ...
  editBtn.addEventListener('click', () => {
    ...
    const rebuild = () => {
      ...
      for (const [key, translationKey, fallback, glyph] of SCOPES) {
        menu.appendChild(
          menuItem(
            editText(translationKey, fallback),
            picked.has(key),
...
function setupZoomMenu() {
  ...
  btn.addEventListener('click', () => {
    ...
    menu.appendChild(
      zoomRow(
        editText('fitToPage', 'Fit to Page'),
        false,
        'ph-arrows-out',
        () => {
          fitPage();
    ...
    menu.appendChild(
      zoomRow(
        editText('fitToWidth', 'Fit to Width'),
        false,
    ...
    inp.min = '20';
    inp.max = '600';
    inp.placeholder = editText('customZoom', 'Custom Zoom');
...
function relocateAddText() {
  ...
  addTextBtn.title = editText('addTextBox', 'Add a text box');
...
function setupDocPanel() {
  ...
  btn.addEventListener('click', () => {
    ...
    title.textContent = editText('documentProperties', 'Document Properties');
    ...
    const groups: [string, [string, string][]][] = [
      [
        ...
        [
          [editText('file', 'File'), desc.meta.name],
          [editText('title', 'Title'), f.Title || '—'],
          [editText('author', 'Author'), f.Author || '—'],
          [editText('subject', 'Subject'), f.Subject || '—'],
          [editText('keywords', 'Keywords'), f.Keywords || '—'],
          [
            editText('created', 'Created'),
            f.CreationDate ? formatPdfDate(f.CreationDate) : '—',
          ],
          [
            editText('modified', 'Modified'),
            f.ModDate ? formatPdfDate(f.ModDate) : '—',
          ],
          [editText('application', 'Application'), f.Creator || '—'],
        ],
      ],
      [
        editText('advanced', 'Advanced'),
        [
          [editText('pdfProducer', 'PDF Producer'), f.Producer || '—'],
          [
            editText('fileSize', 'File Size'),
            `${(desc.meta.size / 1024).toFixed(0)} KB`,
          ],
          [
            editText('pageSize', 'Page Size'),
            desc.pageSize
              ? `${inch(desc.pageSize.w)} × ${inch(desc.pageSize.h)} in`
              : '—',
          ],
          [
            editText('numberOfPages', 'Number of Pages'),
            desc.info?.pages ? String(desc.info.pages) : '—',
          ],
          ...(desc.info?.signatures
            ? ([
                [
                  editText('signatures', 'Signatures'),
                  String(desc.info.signatures),
                ],
              ] as [string, string][])
            : []),
          ...(desc.pdfa
            ? ([[editText('conformance', 'Conformance'), 'PDF/A']] as [
                string,
                string,
              ][])
            : []),
...
async function launchEditor(file: File) {
  if (launching) return;
  launching = true;
  showLoader(editText('loadingEditor', 'Loading PDF Text Editor...'));
  try {
    hideLoader();
...
function handleFiles(files: FileList) {
  ...
  if (!pdf) {
    showAlert(
      editText('invalidFile', 'Invalid File'),
      editText('validPdfRequired', 'Please upload a valid PDF file.')
    );
    return;
  }
