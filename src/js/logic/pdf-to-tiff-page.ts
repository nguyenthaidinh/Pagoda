...
import { joinTiffPages } from '../utils/tiff-pages.js';
...
async function convert() {
  ...
  try {
    ...
    hideLoader();
    showAlert(t('common.error'), t('tools:pdfToTiff.processorLoadError'));
    return;
  }
  ...
  try {
    ...
    if (options.multiPage && pdf.numPages > 1) {
      ...
      for (let i = 1; i <= pdf.numPages; i++) {
        ...
        pages.push(encodePageToTiff(vips, rgba, width, height, options));
      }
      const blob = new Blob([joinTiffPages(pages)], { type: 'image/tiff' });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const handleFileSelect = (newFiles: FileList | null) => {
    ...
    const { validFiles } = filterPdfFiles(newFiles);
