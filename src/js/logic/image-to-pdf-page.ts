...
import {
  conversionText,
  goToLocalizedTools,
} from '@/js/utils/conversion-page.js';
...
function handleFiles(newFiles: FileList) {
  ...
  if (validFiles.length < newFiles.length) {
    showAlert(
      ...
      conversionText('invalidFiles', { formats: IMAGE_FORMATS_LABEL })
    );
  }
...
async function convertToPdf() {
  if (files.length === 0) {
    showAlert(
      ...
      conversionText('noFiles', { format: conversionText('imageFiles') })
    );
    return;
  }
  ...
  try {
    ...
    showLoader(
      conversionText('converting', { format: conversionText('imageFiles') })
    );
    ...
    showAlert(
      ...
      e instanceof Error
        ? conversionText('failedWithReason', {
            format: conversionText('imageFiles'),
            message: e.message,
          })
        : conversionText('failed', { format: conversionText('imageFiles') })
    );
  } finally {
    hideLoader();
  }
