...
async function convert() {
  ...
  try {
    ...
    showLoader(pdfExportText('extractingTables'));
    ...
    try {
      for (let i = 0; i < pageCount; i++) {
        showLoader(
          pdfExportText('scanningPage', { current: i + 1, total: pageCount })
        );
    ...
    if (allTables.length === 0) {
      showAlert(pdfExportText('noTablesTitle'), pdfExportText('noTables'));
      return;
    }

    showLoader(pdfExportText('creatingExcel'));
    ...
    showAlert(
      ...
      pdfExportText('tablesExtracted', { count: allTables.length }),
      'success',
      resetState
    );
  } catch (e) {
    ...
    showAlert(
      ...
      pdfExportText('conversionFailed', { format: 'Excel', message })
    );
  } finally {
    hideLoader();
  }
