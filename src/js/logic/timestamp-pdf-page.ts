...
async function updatePdfDisplay(): Promise<void> {
  ...
  metaSpan.textContent = `${formatBytes(state.pdfFile.size)} \u2022 ${securityText('loadingPages')}`;
  ...
  try {
    if (state.pdfBytes) {
      ...
      metaSpan.textContent = `${formatBytes(state.pdfFile.size)} \u2022 ${securityText('pageCount', { count: pdfDoc.numPages })}`;
...
async function processTimestamp(): Promise<void> {
  ...
  showLoader(t('tools:timestampPdf.applyingTimestamp'));
  ...
  try {
    ...
    downloadFile(blob, pdfOutputName(state.pdfFile.name, 'timestamped'));
    ...
    const message =
      error instanceof Error ? error.message : securityText('unknownError');
    showAlert(
      t('tools:timestampPdf.failureTitle'),
      t('tools:timestampPdf.failureMessage', { message })
    );
  } finally {
    hideLoader();
  }
