    ...
    try {
      if (state.files.length === 0) {
        showAlert(
          ...
          conversionText('noFiles', { format: TOOL_NAME })
        );
        return;
      }
      ...
      if (state.files.length === 1) {
        ...
        showLoader(
          conversionText('convertingFile', { file: originalFile.name })
        );
        ...
        showLoader(conversionText('converting', { format: TOOL_NAME }));
  ...
  const handleFileSelect = (files: FileList | readonly File[] | null) => {
    ...
    if (rejectedCount > 0) {
      showAlert(
        ...
        conversionText('invalidFiles', { formats: TOOL_NAME })
      );
    }
