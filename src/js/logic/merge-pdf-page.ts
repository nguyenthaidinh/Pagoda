...
import { isValidMergePageRange } from '../utils/merge-pdf-helpers.js';
...
async function renderPageMergeThumbnails() {
  ...
  try {
      ...
      await renderPagesProgressively(
        ...
        createWrapperWithFileName,
        {
          createPlaceholder: (pageNumber) => {
            const placeholder = createPlaceholder(pageNumber, fileKey);
            placeholder.dataset.pageIndex = (pageNumber - 1).toString();
            return placeholder;
          },
          batchSize: 8,
          useLazyLoading: true,
          ...
          onProgress: () => {
            showLoader(t('tools:mergePdf.renderingPreviews'));
          },
          onBatchComplete: () => {
            createIcons({ icons });
            initPagePreview(container, pdfjsDoc, {
              resolveDocument: (thumbnail) =>
                mergeState.pdfDocs[thumbnail.dataset.fileName ?? ''],
            });
          },
        }
      );
    ...
    showAlert(t('common.error'), t('tools:mergePdf.renderFailed'));
  } finally {
    hideLoader();
...
export async function merge() {
  if (isMerging || mergeState.isRendering) return;
  ...
  try {
    ...
    if (mergeState.activeMode === 'file') {
      ...
      for (const fileKey of sortedFileKeys) {
        ...
        if (rangeInput && rangeInput.value.trim()) {
          const pageCount = mergeState.pdfDocs[fileKey]?.numPages ?? 0;
          if (!isValidMergePageRange(rangeInput.value, pageCount)) {
            showAlert(
              t('common.error'),
              t('tools:mergePdf.invalidPageRange', {
                file: fileKey.replace(/^\d+_/, ''),
                count: pageCount,
              })
            );
            hideLoader();
    ...
    if (jobs.length === 0) {
      showAlert(t('common.error'), t('tools:mergePdf.noSelection'));
      hideLoader();
      return;
    }
    ...
    const transferableFiles = filesToMerge.map((file) => ({
      name: file.name,
    ...
    mergeWorker.onmessage = (e: MessageEvent<MergeResponse>) => {
      ...
      if (e.data.status === 'success') {
        ...
        showAlert(
          t('common.error'),
          e.data.message || t('tools:mergePdf.mergeFailed')
        );
      }
    };
    ...
    mergeWorker.onerror = (e) => {
      ...
      showAlert(t('common.error'), t('tools:mergePdf.workerFailed'));
    };
    ...
    showAlert(t('common.error'), t('tools:mergePdf.invalidFiles'));
    hideLoader();
  } finally {
    if (!workerStarted) finishMerge();
  }
}
...
export async function refreshMergeUI() {
  ...
  try {
    ...
    showAlert(t('common.error'), t('tools:mergePdf.loadFailed'));
    return;
  } finally {
    hideLoader();
  }
  ...
  (state.files as File[]).forEach((f, index) => {
    ...
    label.textContent = t('tools:mergePdf.pageRangeLabel', {
      count: pageCount,
    });
    ...
    input.placeholder = t('tools:mergePdf.allPagesPlaceholder');
    ...
    deleteBtn.title = t('tools:mergePdf.removeFile');
    deleteBtn.onclick = (e) => {
      e.stopPropagation();
