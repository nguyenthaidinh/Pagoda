...
import { filterPdfFiles } from '../utils/pdf-export-page.js';
...
pdfFilesInput.addEventListener('change', (e) => {
  ...
  if (target.files && target.files.length > 0) {
    const { validFiles, rejectedCount } = filterPdfFiles(target.files);
    selectedFiles = validFiles;
    convertBtn.disabled = selectedFiles.length === 0;
    updateFileList();

    if (selectedFiles.length === 0) {
      showStatus(t('tools:pdfExport.invalidFiles'), 'error');
