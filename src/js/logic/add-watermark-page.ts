...
async function handleFiles(files: FileList) {
  ...
  if (!file || !isPdfFile(file)) {
    showAlert(t('common.error'), t('tools:addWatermark.invalidFile'));
    return;
  }
  try {
    const result = await loadPdfWithPasswordPrompt(file);
    if (!result) return;
    showLoader(t('tools:addWatermark.loading'));
    ...
    if (editorFileInfo) {
      editorFileInfo.textContent = `${file.name} (${t(
        'tools:addWatermark.fileMeta',
        {
    ...
    console.error(error);
    showAlert(t('common.error'), t('tools:addWatermark.loadFailed'));
  } finally {
    hideLoader();
  }
}
...
function updateFileDisplay() {
  ...
  metaSpan.textContent = t('tools:addWatermark.fileMeta', {
  ...
  removeBtn.title = t('tools:addWatermark.removeFile');
...
function setupEditorControls() {
  ...
  imageInput?.addEventListener('change', () => {
    ...
    if (!isSupportedImage) {
      imageInput.value = '';
      showAlert(t('common.error'), t('tools:addWatermark.unsupportedImage'));
      return;
    }
...
async function applyWatermark() {
  if (!pageState.pdfDoc || !pageState.pdfBytes) {
    showAlert(t('common.error'), t('tools:addWatermark.uploadFirst'));
    return;
  }

  let flattenedSource: pdfjsLib.PDFDocumentProxy | null = null;
  try {
    saveCurrentPageConfig();
    ...
    if (applyToAllPages) {
      ...
      const pageIndices =
        pageRangeStr.toLowerCase() === 'all'
          ...
          : parsePageRangesStrict(
              pageRangeStr,
              pageState.pdfDoc.getPageCount()
            );

      if (pageIndices?.length === 0) {
        showAlert(t('common.error'), t('tools:addWatermark.invalidRange'));
        return;
      }
      ...
      if (config.type === 'text') {
        ...
        if (!text.trim()) {
          showAlert(t('common.error'), t('tools:addWatermark.textRequired'));
          return;
        }
        ...
        if (!imageFile) {
          showAlert(t('common.error'), t('tools:addWatermark.imageRequired'));
          return;
        }
        const imageBytes = await readFileAsArrayBuffer(imageFile);

        let imageType: 'png' | 'jpg';
        if (imageFile.type === 'image/png' || /\.png$/i.test(imageFile.name)) {
          imageType = 'png';
          ...
          showAlert(
            t('common.error'),
            t('tools:addWatermark.unsupportedImage')
          );
          return;
        }
      ...
      for (const { config, indices } of configGroups.values()) {
        ...
        if (config.type === 'text') {
          ...
          let imageType: 'png' | 'jpg';
          if (
            config.imageFile.type === 'image/png' ||
            /\.png$/i.test(config.imageFile.name)
          ) {
            imageType = 'png';
    ...
    showLoader(t('tools:addWatermark.processing'));

    const flattenCheckbox = document.getElementById(
      'flatten-watermark'
    ) as HTMLInputElement;
    if (flattenCheckbox?.checked) {
      flattenedSource = await getPDFDocument({
        data: resultBytes.slice(),
      }).promise;
      const flattenedDoc = await PDFLibDocument.create();
      const totalPages = flattenedSource.numPages;
      const renderScale = 2.5;

      for (let i = 1; i <= totalPages; i++) {
        showLoader(
          t('tools:addWatermark.flattening', {
            current: i,
            total: totalPages,
          })
        );
        const page = await flattenedSource.getPage(i);
    ...
    showAlert(t('common.success'), t('tools:addWatermark.success'), 'success');
  } catch (e: unknown) {
    console.error(e);
    showAlert(t('common.error'), t('tools:addWatermark.failed'));
  } finally {
    flattenedSource?.destroy();
    hideLoader();
  }
}
