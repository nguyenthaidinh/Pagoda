...
async function convert() {
  if (files.length === 0) {
    showAlert(
      ...
      conversionText('noFiles', { format: 'TIFF' })
    );
    return;
  }
  ...
  showLoader(conversionText('converting', { format: 'TIFF' }));
  try {
    ...
    for (const file of files) {
      ...
      for (const ifd of ifds) {
        ...
        const rgba = tiffIfdToRgba(
          ...
          ifd.type,
          ifd.bitsPerSample,
          ifd.type === 3 ? ifd.palette : undefined
        );
    ...
    showAlert(
      ...
      conversionText('failed', { format: 'TIFF' })
    );
  } finally {
    hideLoader();
  }
  ...
  const handleFileSelect = (newFiles: FileList | null) => {
    ...
    if (validFiles.length < newFiles.length) {
      showAlert(
        ...
        conversionText('invalidFiles', { formats: 'TIFF' })
      );
    }
