...
async function handleFile(file: File) {
  if (
    ...
    showAlert(t('common.error'), t('tools:extractPages.invalidFile'));
    return;
  }
  ...
  try {
    ...
    showLoader(t('tools:extractPages.loadingPdf'));
    extractState.file = result.file;
    result.pdf.destroy();
    extractState.pdfDoc = await loadPdfDocument(result.bytes);
    extractState.sourceBytes = new Uint8Array(result.bytes.slice(0));
    ...
    hideLoader();
    showAlert(t('common.error'), t('tools:extractPages.loadFailed'));
  }
}
...
function updateFileDisplay() {
  ...
  metaSpan.textContent = `${formatBytes(extractState.file.size)} • ${extractState.totalPages} ${t('common.pages')}`;
...
async function extractPages() {
  if (!extractState.sourceBytes) return;
  const pagesInput = document.getElementById(
    'pages-to-extract'
  ) as HTMLInputElement;
  if (!pagesInput || !pagesInput.value.trim()) {
    showAlert(
      t('tools:extractPages.noPagesTitle'),
      t('tools:extractPages.noPages')
    );
    return;
  }
  ...
  if (pagesToExtract.length === 0) {
    showAlert(
      t('tools:extractPages.invalidPagesTitle'),
      t('tools:extractPages.invalidPages')
    );
    return;
  }

  showLoader(t('tools:extractPages.extracting'));

  try {
    const zip = new JSZip();
    const baseName =
      extractState.file?.name.replace(/\.pdf$/i, '') || 'document';

    const qpdf = await initializeQpdf();
    showLoader(t('tools:extractPages.extracting'));
    const inputPath = '/extract-pages-input.pdf';
    qpdf.FS.writeFile(inputPath, extractState.sourceBytes);
    try {
      for (const pageNum of pagesToExtract) {
        const pdfBytes = extractPagesWithQpdf(qpdf, inputPath, [pageNum - 1]);
    ...
    showAlert(
      t('common.success'),
      t('tools:extractPages.extracted', { count: pagesToExtract.length }),
      'success',
      () => {
        resetState();
      }
    ...
    hideLoader();
    showAlert(t('common.error'), t('tools:extractPages.failed'));
  }
}
