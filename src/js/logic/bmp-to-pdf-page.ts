...
async function convert() {
  if (files.length === 0) {
    showAlert(
      ...
      conversionText('noFiles', { format: 'BMP' })
    );
    return;
  }
  showLoader(conversionText('converting', { format: 'BMP' }));
  try {
    ...
    showAlert(
      ...
      conversionText('failed', { format: 'BMP' })
    );
  } finally {
    hideLoader();
  }
  ...
  const handleFileSelect = (newFiles: FileList | null) => {
    ...
    if (validFiles.length < newFiles.length) {
      showAlert(
        ...
        conversionText('invalidFiles', { formats: 'BMP' })
      );
    }
