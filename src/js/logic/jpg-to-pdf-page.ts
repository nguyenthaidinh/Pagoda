...
import { goToLocalizedTools } from '../utils/conversion-page.js';
...
function handleFiles(newFiles: FileList) {
  ...
  if (validFiles.length < newFiles.length) {
    showAlert(
      t('tools:jpgToPdf.invalidFilesTitle'),
      t('tools:jpgToPdf.invalidFiles')
    );
  }
...
async function convertToPdf() {
  if (files.length === 0) {
    showAlert(t('tools:jpgToPdf.noFilesTitle'), t('tools:jpgToPdf.noFiles'));
    return;
  }

  showLoader(t('tools:jpgToPdf.loadingEngine'));

  try {
    const mupdf = await ensurePyMuPDF();

    showLoader(t('tools:jpgToPdf.converting'));
    ...
    showAlert(
      t('tools:jpgToPdf.conversionError'),
      t('tools:jpgToPdf.conversionFailed')
    );
  } finally {
    hideLoader();
  }
