...
async function handleFile(file: File) {
  if (
    ...
    showAlert(t('common.error'), t('tools:signPdf.invalidFile'));
    return;
  }

  signState.file = file;
  try {
    const ready = await updateFileDisplay();
    if (ready) await setupSignTool();
  } catch (error) {
    console.error('Failed to load the PDF signing tool:', error);
    hideLoader();
    showAlert(t('common.error'), t('tools:signPdf.loadFailed'));
    resetState();
  }
}

async function updateFileDisplay(): Promise<boolean> {
  const fileDisplayArea = document.getElementById('file-display-area');

  if (!fileDisplayArea || !signState.file) return false;
  ...
  metaSpan.textContent = `${formatBytes(result.file.size)} • ${result.pdf.numPages} ${t('common.pages')}`;
  result.pdf.destroy();
  return true;
}
...
async function setupSignTool() {
  ...
  showLoader(t('tools:signPdf.loadingViewer'));
  ...
  if (signState.blobUrl) URL.revokeObjectURL(signState.blobUrl);
...
async function applyAndSaveSignatures() {
  if (!signState.viewerReady || !signState.viewerIframe) {
    showAlert(
      t('tools:signPdf.viewerNotReadyTitle'),
      t('tools:signPdf.viewerLoading')
    );
    return;
  }
  ...
  try {
    ...
    if (!viewerWindow || !viewerWindow.PDFViewerApplication) {
      showAlert(
        t('tools:signPdf.viewerNotReadyTitle'),
        t('tools:signPdf.viewerInitializing')
      );
      return;
    }
    ...
    if (shouldFlatten) {
      showLoader(t('tools:signPdf.flattening'));
      ...
      hideLoader();
      showAlert(t('common.success'), t('tools:signPdf.saved'), 'success', () =>
        resetState()
      );
    } else {
    ...
    hideLoader();
    showAlert(
      t('tools:signPdf.exportFailedTitle'),
      t('tools:signPdf.exportFailed')
    );
  }
}
