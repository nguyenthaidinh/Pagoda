...
const updateUI = () => {
  ...
  if (files.length > 0) {
    ...
    files.forEach((file) => {
      ...
      removeBtn.title = t('tools:adjustColors.removeFile');
      removeBtn.onclick = () => {
        files = [];
...
async function processAllPages(): Promise<void> {
  if (files.length === 0) {
    showAlert(t('common.error'), t('tools:adjustColors.uploadFirst'));
    return;
  }

  showLoader(t('tools:adjustColors.processing'));
  ...
  try {
    ...
    for (let i = 1; i <= doc.numPages; i++) {
      showLoader(
        t('tools:adjustColors.processingPage', {
          current: i,
          total: doc.numPages,
        })
      );
      ...
      const pngBlob = await canvasToBlob(outputCanvas);
    ...
    console.error(e);
    showAlert(t('common.error'), t('tools:adjustColors.failed'));
  } finally {
    doc?.destroy();
    hideLoader();
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const handleFileSelect = async (newFiles: FileList | null) => {
    ...
    if (validFiles.length === 0) {
      showAlert(t('common.error'), t('tools:adjustColors.invalidFile'));
      return;
    }
    ...
    try {
      ...
      if (!result) return;
      showLoader(t('tools:adjustColors.loadingPreview'));
      clearPreviewDocument();
      files = [result.file];
      pdfjsDoc = result.pdf;
      updateUI();
      ...
      console.error(e);
      showAlert(t('common.error'), t('tools:adjustColors.loadFailed'));
    } finally {
      hideLoader();
    }
  };
