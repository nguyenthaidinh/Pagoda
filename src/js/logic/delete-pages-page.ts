...
const deleteState: DeletePagesState & { sourceBytes: Uint8Array | null } = {
  file: null,
  pdfDoc: null,
  sourceBytes: null,
  pdfJsDoc: null,
...
async function handleFile(file: File) {
  if (
    ...
    showAlert(t('common.error'), t('tools:deletePages.invalidFile'));
    return;
  }
  ...
  try {
    ...
    showLoader(t('tools:deletePages.loadingPdf'));
    deleteState.file = result.file;
    deleteState.pdfDoc = await loadPdfDocument(result.bytes);
    deleteState.sourceBytes = new Uint8Array(result.bytes.slice(0));
    ...
    hideLoader();
    showAlert(t('common.error'), t('tools:deletePages.loadFailed'));
  }
}
...
function updateFileDisplay() {
  ...
  metaSpan.textContent = `${formatBytes(deleteState.file.size)} • ${deleteState.totalPages} ${t('common.pages')}`;
...
function updatePreview() {
  ...
  const inputValue = pagesInput.value.trim();
  deleteState.pagesToDelete = inputValue
    ? new Set(
        parsePageRanges(inputValue, deleteState.totalPages).map((i) => i + 1)
      )
    : new Set();
...
async function deletePages() {
  if (deleteState.pagesToDelete.size === 0) {
    showAlert(
      t('tools:deletePages.noPagesTitle'),
      t('tools:deletePages.noPages')
    );
    return;
  }

  if (deleteState.pagesToDelete.size >= deleteState.totalPages) {
    showAlert(t('common.error'), t('tools:deletePages.cannotDeleteAll'));
    return;
  }

  showLoader(t('tools:deletePages.deleting'));

  try {
    if (!deleteState.sourceBytes) return;
    const keptPages = Array.from(
      { length: deleteState.totalPages },
      (_, i) => i
    ).filter((index) => !deleteState.pagesToDelete.has(index + 1));
    const qpdf = await initializeQpdf();
    showLoader(t('tools:deletePages.deleting'));
    const inputPath = '/delete-pages-input.pdf';
    qpdf.FS.writeFile(inputPath, deleteState.sourceBytes);
    let resultBytes: Uint8Array;
    try {
      resultBytes = extractPagesWithQpdf(qpdf, inputPath, keptPages);
    } finally {
    ...
    showAlert(
      ...
      t('tools:deletePages.deleted', {
        count: deleteState.pagesToDelete.size,
      }),
      'success',
      () => resetState()
    );
    ...
    hideLoader();
    showAlert(t('common.error'), t('tools:deletePages.failed'));
  }
}

function resetState() {
  deleteState.pdfJsDoc?.destroy();
  deleteState.file = null;
  deleteState.pdfDoc = null;
  deleteState.sourceBytes = null;
