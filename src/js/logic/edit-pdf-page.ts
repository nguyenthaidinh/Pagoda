...
import { vietnameseEmbedPdfLocale } from '../i18n/embedpdf-vi';
...
async function handleFiles(files: FileList) {
  ...
  if (pdfFiles.length === 0) {
    showAlert(t('common.error'), t('tools:pdfEditor.invalidFile'));
    return;
  }

  showLoader(t('tools:pdfEditor.loadingEditor'));
  ...
  try {
    ...
    if (!isViewerInitialized) {
      ...
      const editorLanguage = document.documentElement.lang;
      viewerInstance = EmbedPDF.init({
        ...
        i18n: {
          defaultLocale: editorLanguage.startsWith('vi')
            ? 'vi'
            : editorLanguage.startsWith('de')
              ? 'de'
              : 'en',
          ...(editorLanguage.startsWith('vi') && {
            locales: [vietnameseEmbedPdfLocale],
          }),
        },
        documentManager: {
          maxDocuments: 10,
      ...
      if (!downloadBtn) {
        ...
        downloadBtn.textContent = t('tools:pdfEditor.downloadEdited');
      ...
      downloadBtn.onclick = async () => {
        try {
          ...
          showAlert(t('common.error'), t('tools:pdfEditor.downloadFailed'));
        }
      };
    ...
    showAlert(t('common.error'), t('tools:pdfEditor.loadFailed'));
  } finally {
    hideLoader();
  }
}
