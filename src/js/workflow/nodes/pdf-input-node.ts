...
import { isPdfPasswordProtected } from '../../utils/pdf-encryption.js';
...
export class PDFInputNode extends BaseWorkflowNode {
  ...
  async addFile(file: File): Promise<void> {
    ...
    if (await isPdfPasswordProtected(arrayBuffer as ArrayBuffer)) {
      throw new EncryptedPDFError(file.name);
    }

    let document: Awaited<ReturnType<typeof loadPdfDocument>>;
    try {
      document = await loadPdfDocument(bytes);
      if (document.isEncrypted) {
        const decrypted = await decryptPdfBytes(bytes, '');
        bytes = new Uint8Array(decrypted.bytes);
        document = await loadPdfDocument(bytes);
      }
    } catch {
