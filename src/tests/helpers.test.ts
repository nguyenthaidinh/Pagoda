...
describe('helpers', () => {
  ...
  describe('parsePageRangesStrict', () => {
    it('returns all pages when the range is blank', () => {
      expect(parsePageRangesStrict('', 4)).toEqual([0, 1, 2, 3]);
    });

    it('parses valid ranges, removes duplicates, and sorts pages', () => {
      expect(parsePageRangesStrict('5, 1-3, 3', 5)).toEqual([0, 1, 2, 4]);
    });

    it('rejects the entire range when any token is invalid', () => {
      expect(parsePageRangesStrict('1, nope, 3', 5)).toEqual([]);
      expect(parsePageRangesStrict('3-2', 5)).toEqual([]);
      expect(parsePageRangesStrict('1-8', 5)).toEqual([]);
    });
  });

  describe('isPdfFile', () => {
    it('accepts PDF MIME types and safe extension fallbacks', () => {
      expect(
        isPdfFile(
          new File(['pdf'], 'document.bin', { type: 'application/pdf' })
        )
      ).toBe(true);
      expect(isPdfFile(new File(['pdf'], 'document.PDF'))).toBe(true);
      expect(
        isPdfFile(
    ...
    it('rejects non-PDF files even when their extension is misleading', () => {
      expect(
        isPdfFile(new File(['image'], 'image.pdf', { type: 'image/png' }))
      ).toBe(false);
      expect(
        isPdfFile(new File(['text'], 'notes.txt', { type: 'text/plain' }))
      ).toBe(false);
    });
  });
});
