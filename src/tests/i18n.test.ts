...
import { getLanguageFromUrl, getLocalizedPath } from '@/js/i18n/i18n';
...
describe('getLocalizedPath', () => {
  it('keeps the same page while changing locale', () => {
    expect(getLocalizedPath('vi', '/merge-pdf.html', '')).toBe(
      '/vi/merge-pdf.html'
    );
    expect(getLocalizedPath('de', '/vi/merge-pdf.html', '')).toBe(
      '/de/merge-pdf.html'
    );
    expect(getLocalizedPath('en', '/de/merge-pdf.html', '')).toBe(
      '/merge-pdf.html'
    );
  });

  it('preserves the home page and deployment base path', () => {
    expect(getLocalizedPath('vi', '/', '')).toBe('/vi/');
    expect(getLocalizedPath('de', '/pagoda/tools.html', '/pagoda')).toBe(
      '/pagoda/de/tools.html'
    );
    expect(getLocalizedPath('en', '/pagoda/vi/', '/pagoda')).toBe('/pagoda/');
  });
});
