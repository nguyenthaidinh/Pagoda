...
describe('split-pdf-helpers (pure planning)', () => {
  ...
  describe('bookmarkSplitGroups', () => {
    ...
    it('keeps the preface separate when the first bookmark starts later', () => {
      expect(bookmarkSplitGroups([3], 5)).toEqual([
        [0, 1, 2],
        [3, 4],
      ]);
    });

    it('splits at every boundary including the first', () => {
    ...
    it('does not create empty files for duplicate or out-of-range bookmarks', () => {
      expect(bookmarkSplitGroups([0, 2, 2, -1, 5, 99], 5)).toEqual([
        [0, 1],
        [2, 3, 4],
      ]);
    });
...
describe('split modes end-to-end with real qpdf', () => {
  ...
  it('reorders, duplicates and removes pages without losing the document title', async () => {
    const source = await PDFDocument.load(await makePdf(4));
    source.setTitle('Original document title');
    const output = extract(await source.save(), [2, 0, 0]);
    expect(await originalIndices(output)).toEqual([2, 0, 0]);
    expect((await PDFDocument.load(output)).getTitle()).toBe(
      'Original document title'
    );
  });

  it('keeps editable form data on retained pages', async () => {
    const source = await PDFDocument.load(await makePdf(3));
    const field = source.getForm().createTextField('name');
    field.setText('PagodaPDF');
    field.addToPage(source.getPage(2), { x: 10, y: 10, width: 60, height: 20 });
    const output = await PDFDocument.load(extract(await source.save(), [2, 0]));
    expect(output.getForm().getTextField('name').getText()).toBe('PagodaPDF');
    expect(output.getPage(0).node.Annots()?.size()).toBe(1);
  });
